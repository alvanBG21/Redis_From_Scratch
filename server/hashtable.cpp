...
// hashtable loop up subroutine
// pay attention ti the return value. It returns the address of
...
{
    size_t nwork = 0;
    while (nwork < k_rehashing_work && hmap->older.size)
    {
        // find non-empty slot
...
static bool h_foreach(HTab *htab, bool (*f)(HNode *,  void *), void *arg){
    for(size_t i =0; htab->size != 0 && i <= htab->mask; i++){
